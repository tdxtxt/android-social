 ...
 * 压缩图片之后的返回结果
 */
abstract class ThumbDataContinuation protected constructor(private val tag: String, private val msg: String, private val listener: OnShareListener?) : Continuation<ByteArray, Any> {

    @Throws(Exception::class)
    override fun then(task: Task<ByteArray>): Any? {
        if (task.isFaulted || task.result == null) {
            SocialLogUtils.e(tag, "图片压缩失败 -> $msg")
            listener?.onFailure(SocialError(SocialError.CODE_IMAGE_COMPRESS_ERROR, msg).exception(task.error))
        } else {
