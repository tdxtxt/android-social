...
object SocialGoUtils {
    ...
    private val TAG = SocialGoUtils::class.java.simpleName
    ...
    private fun getMaxSizeBitmap(filePath: String, maxSize: Int): Bitmap {
        ...
        SocialLogUtils.e(TAG, "sample size = " + sampleSize + " 采样后 bitmap大小 = " + bitmap.byteCount)
        return bitmap
    }
    ...
    private fun getStaticSizeBitmapByteByBitmap(srcBitmap: Bitmap, maxSize: Int, format: Bitmap.CompressFormat): ByteArray {
        ...
        SocialLogUtils.e(TAG, "开始循环压缩之前 bytes = " + bytes.size)
        while (bytes.size > maxSize) {
            ...
            SocialLogUtils.e(TAG, "压缩一次 bytes = " + bytes.size)
        }
        SocialLogUtils.e(TAG, "压缩后的图片输出大小 bytes = " + bytes.size)
    ...
    fun getStaticSizeBitmapByteByPathTask(path: String, maxSize: Int): Task<ByteArray> {
        return Task.callInBackground { getStaticSizeBitmapByteByPath(path, maxSize) }
    }
    ...
    fun mapUrl2LocalPath(url: String): String {
        ...
        val saveFile = File(SocialSdk.getConfig().getCacheDir(), fileName)
    ...
    fun <T> startJsonRequest(url: String, clz: Class<T>, callback: Callback<T>) {
        Task.callInBackground {
            var any: T? = null
            val json = SocialSdk.getRequestAdapter().getJson(url)
            if (!TextUtils.isEmpty(json)) {
                any = getObject(json, clz)
            }
            any
        }.continueWith(Continuation<T?, Boolean> { task ->
            if (!task.isFaulted && task.result != null) {
                callback.onSuccess(task.result)
            } else if (task.isFaulted) {
                callback.onFailure(SocialError(SocialError.CODE_REQUEST_ERROR, task.error))
            } else if (task.result == null) {
                callback.onFailure(SocialError(SocialError.CODE_PARSE_ERROR, "json 无法解析"))
            } else {
                callback.onFailure(SocialError(SocialError.CODE_REQUEST_ERROR, "unKnow error"))
            }
            true
        }, Task.UI_THREAD_EXECUTOR).continueWith { task ->
            if (task.isFaulted) {
                callback.onFailure(SocialError(SocialError.CODE_REQUEST_ERROR, "未 handle 的错误"))
            }
            null
        }
    }
