    ...
    private fun getUserInfo(qqToken: QQAccessToken?) {
        ...
        info.getUserInfo(object : IUiListener {
            override fun onComplete(any: Any) {
                ...
                if (qqUserInfo == null) {
                    listener?.onFailure(SocialError(SocialError.CODE_PARSE_ERROR, "#getUserInfo#解析 qq user 错误, data = " + any.toString()))
                } else {
                    qqUserInfo.setOpenId(tencentApi.openId)
                    listener?.onSuccess(LoginResult(loginType, qqUserInfo, qqToken))
                }
            }
